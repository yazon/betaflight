...
typedef union gyroSoftFilter_u {
    ...
    adaptiveFilter_t gyroFilterAdaptiveState[XYZ_AXIS_COUNT];
} gyroSoftLpfFilter_t;
...
//            break;
//        case FILTER_PT1:
		gyroSensor->softLpfFilterApplyFn = (filterApplyFnPtr)adaptiveFilterApply;
		// const float gyroDt = (float) gyro.targetLooptime * 0.000001f;
		for (int axis = 0; axis < XYZ_AXIS_COUNT; axis++) {
			gyroSensor->softLpfFilterPtr[axis] = (filter_t *)&gyroSensor->softLpfFilter.gyroFilterAdaptiveState[axis];
